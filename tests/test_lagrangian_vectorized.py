...
def drive(net, fluid, soil, mass_flows, node_temps, ts_ids):
    ...
    for mdot, t_nodes, ts in zip(mass_flows, node_temps, ts_ids):
        ...
        COMPONENT_FUNCTIONS_DICT["lagrangian_pipe"]["delta_p"](net, fluid, mask=mask)
...
class LagrangianVectorEquivalence(unittest.TestCase):
    def assert_equivalent(self, net, fluid, soil, mass_flows, node_temps, ts_ids):
        ...
        for k, ((outs_v, states_v), (outs_s, states_s)) in enumerate(zip(rec_v, rec_s)):
    ...
    def test_full_simulation(self):
        ...
        for i in (1, 2):
            net.add_consumer(
                ...
                setpoint_value_hyd=0.3,
                setpoint_type_hx="delta_q",
                setpoint_value_hx=-2.0,
        ...
        def run(net):
            ...
            return [
                loop.execute(net=net, fluid=fluid, soil=soil, ts_id=k) for k in range(5)
            ]
